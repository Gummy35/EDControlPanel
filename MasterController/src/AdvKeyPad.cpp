...
uint8_t AdvKeyPad::_getKeyPad()
{
  ...
  {
    if (KP__REVERSE)
      rowmask = 0xff - (1 << 7 - row);
    else
      rowmask = 0xff - (1 << row + 4);
    ...
    for (int col = 0; col < 4; col++)
    {
      ...
      { // Key pressed
        ...
        else
        {
          ...
          {
            if (millis() > _trigger[keynum] + _debounce)
